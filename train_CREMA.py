...
from datasets.CREMA import CramedDataset
...
def train_withP(epoch, train_loader, model, optimizer, logger,state):
    ...
    for step, (spectrogram, image, y) in enumerate(train_loader):
        ...
        modules_video = [m for n, m in model.video_encoder.named_modules() if hasattr(
            m, 'weight') and not bool(re.match('last', n))]
        modules_video = modules_video + [m for n, m in model.cls_v.named_modules() if hasattr(
            m, 'weight') and not bool(re.match('last', n))]
        modules_audio = [m for n, m in model.audio_encoder.named_modules() if hasattr(
            m, 'weight') and not bool(re.match('last', n))]
        modules_audio = modules_audio + [m for n, m in model.cls_a.named_modules() if hasattr(
            m, 'weight') and not bool(re.match('last', n))]
        if state == 'train_audio':
